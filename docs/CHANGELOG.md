...
- [新功能] 每日分析增加 CC Fear And Greed 日线辅助指标，保留 10/30/2 与不重绘一根延迟，区分状态、转向与数据日期；缺失数据不生成信号。
- [修复] 中文分析计划同步观察状态，撤回冲突价位，按入场价计算收益风险比，移除无账户依据的仓位与未校准归因权重；价位列表支持换行。

- [新功能] 每日股票分析自动读取最近完整交易日的已核验短线候选，与原自选股合并去重；选股失败、过期或附件不可用时保留原名单，并在Actions摘要记录联动结果。

- [修复] 独立短线选股提高多股JSON输出预算和超时上限，并记录不含正文的LLM结束原因，帮助诊断空响应或截断；保留AI失败时停止输出候选的行为。

- [新功能] 新增独立短线收盘选股工作流，复用内置筛选引擎与LLM/PushPlus配置，加入最多5只候选、日K日期/价格核验和失败/空名单区分；不修改原自选股定时任务。
