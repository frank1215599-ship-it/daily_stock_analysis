...
中文每日分析在现有阶段护栏后统一校验交易计划，传统和 Agent 管线均适用：

- 观察、非盘中或核心数据不完整时，将入场文字统一为未触发的条件预案。
- 入场、止损、第一目标必须各有一个明确正价格，且止损 < 入场 < 目标；备选入场不能在止损以下。多价位、区间或缺失不能可靠解析时撤回交易预案，不猜测模型意图。
- 对通过校验的首选/备选独立情景计算收益风险比，非胜率，未计费用、跳空和滑点。校验价格顺序不等于核实价格来源或验证策略盈利。
- 不掌握账户资金、持仓和可卖数量时不输出固定仓位或总资金风险上限；不允许把备选入场解释为止损后补仓。
- 新闻证据不完整时舆情标记未知；删除未校准的模型归因百分比。其他量能、估值和数据时间的解释由强化提示词约束，仍需复核模型输出。
- 中文计划时效绑定完整日线日期；下一次检查提示不新增调度任务。
